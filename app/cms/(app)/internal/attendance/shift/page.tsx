...
import {
  ...
  createShiftWorkAttendance,
  updateShiftWorkAttendance,
  deleteShiftWorkAttendance,
...
export default function ShiftWorkAttendancePage() {
  const token = localStorage.getItem("cmsToken");
  const dispatch = useDispatch();
  const { shiftWorkAttendance } = useAttendanceData();
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(listShiftWorkAttendance(token) as any).finally(() =>
      setLoading(false)
    );
  }, [dispatch]);

  const safeShifts = Array.isArray(shiftWorkAttendance)
    ? shiftWorkAttendance
    : [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await dispatch(listShiftWorkAttendance(token) as any);
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await dispatch(
        deleteShiftWorkAttendance({ token, id: deletingShift.id }) as any
      );
      if (res.payload.status === 200 || res.payload.status === 201) {
