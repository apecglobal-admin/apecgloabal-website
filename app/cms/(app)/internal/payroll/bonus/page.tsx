...
export default function BonusPayrollPage() {
  const token = localStorage.getItem("cmsToken");
  const dispatch = useDispatch();
  const { bonus } = usePayrollData();
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(listBonusPayroll(token) as any).finally(() => setLoading(false));
  }, [dispatch]);

  const safebonus = Array.isArray(bonus) ? bonus : [];

  const filteredbonus = useMemo(() => {
    let result = safebonus;
    ...
    return result;
  }, [safebonus, searchDebounce, filterStatus]);

  const totalPages = Math.ceil(filteredbonus.length / limit);
  const paginatedData = filteredbonus.slice((page - 1) * limit, page * limit);

  const totalCount = safebonus.length;
  const activeCount = safebonus.filter((a) => a.active).length;
  ...
  const handleSave = async () => {
    if (!formData.name.trim()) {
      toast.error("Vui lòng nhập tên Thưởng thêm");
      return;
    }
    setCreating(true);
    try {
      ...
      await dispatch(listBonusPayroll(token) as any);
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await dispatch(
        deleteBonusPayroll({ token, id: deletingAllowance.id }) as any
      );
      if (res.payload.status === 200 || res.payload.status === 201) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-white/60 text-xs sm:text-sm mt-0.5">
            Quản lý các khoản Thưởng thêm trong bảng lương
          </p>
        </div>
        <Button
        ...
        >
          ...
          <span className="hidden sm:inline">Thêm Thưởng thêm</span>
        </Button>
      </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="hidden sm:flex gap-2 items-center">
              <Select value={filterStatus} onValueChange={(v: any) => setFilterStatus(v)}>
                ...
                <SelectContent>
                  <SelectItem value="all">Tất cả Thưởng thêm</SelectItem>
      ...
      <Card className="bg-black/50 border-purple-500/30">
        <CardHeader className="pb-3 px-4 sm:px-6 pt-4 sm:pt-6">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus className="h-4 w-4 mr-2" /> Thêm Thưởng thêm
            </Button>
          </div>
        </CardHeader>
              ...
              <div className="sm:hidden space-y-2">
                {paginatedData.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-white/50 text-sm">
                    {hasActiveFilters
                      ? "Không tìm thấy Thưởng thêm nào phù hợp"
                      : "Chưa có Thưởng thêm nào"}
                  </div>
                )}
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-purple-500/30">
                      <TableHead className="text-white">Tên Thưởng thêm</TableHead>
                    ...
                    {paginatedData.length > 0 ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-8 text-white/60">
                          {hasActiveFilters
                            ? "Không tìm thấy Thưởng thêm nào phù hợp với bộ lọc"
                            : "Không tìm thấy Thưởng thêm nào"}
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
        ...
        {totalPages > 1 && (
          <div className="px-3 sm:px-6 pb-4 sm:pb-6">
            <Pagination
              ...
              totalItems={filteredbonus.length}
        ...
        <DialogContent className="bg-black/90 border-purple-500/30 text-white w-[calc(100vw-2rem)] max-w-lg mx-auto rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl font-bold text-white">
              {editingAllowance ? "Chỉnh Sửa Thưởng thêm" : "Thêm Thưởng thêm Mới"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 sm:space-y-5">
            <div>
              ...
              <Input
                id="name"
                placeholder="Nhập tên Thưởng thêm..."
                value={formData.name}
            ...
            >
              {creating ? (
                ...
                <>
                  ...
                  {editingAllowance ? "Cập Nhật" : "Tạo Thưởng thêm"}
                </>
              )}
            </Button>
          </div>
        ...
        <DialogContent className="bg-black/90 border-red-500/30 text-white w-[calc(100vw-2rem)] max-w-md mx-auto rounded-2xl">
          ...
          <div className="space-y-4">
            <p className="text-white/80 text-sm">
              Bạn có chắc chắn muốn xóa Thưởng thêm{" "}
