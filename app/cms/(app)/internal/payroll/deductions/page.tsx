...
import {
  ...
  createDeductionsPayroll,
  updateDeductionsPayroll,
  deleteDeductionsPayroll,
...
export default function DeductionsPayrollPage() {
  const token = localStorage.getItem("cmsToken");
  const dispatch = useDispatch();
  const { deductions } = usePayrollData();
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(listDeductionsPayroll(token) as any).finally(() => setLoading(false));
  }, [dispatch]);

  const safedeductions = Array.isArray(deductions) ? deductions : [];

  const filtereddeductions = useMemo(() => {
    let result = safedeductions;
    ...
    return result;
  }, [safedeductions, searchDebounce, filterStatus]);

  const totalPages = Math.ceil(filtereddeductions.length / limit);
  const paginatedData = filtereddeductions.slice((page - 1) * limit, page * limit);

  const totalCount = safedeductions.length;
  const activeCount = safedeductions.filter((a) => a.active).length;
  ...
  const handleSave = async () => {
    if (!formData.name.trim()) {
      toast.error("Vui lòng nhập tên Khẩu trừ");
      return;
    }
    setCreating(true);
    try {
      ...
      await dispatch(listDeductionsPayroll(token) as any);
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await dispatch(
        deleteDeductionsPayroll({ token, id: deletingAllowance.id }) as any
      );
      if (res.payload.status === 200 || res.payload.status === 201) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-white/60 text-xs sm:text-sm mt-0.5">
            Quản lý các khoản Khẩu trừ trong bảng lương
          </p>
        </div>
        <Button
        ...
        >
          ...
          <span className="hidden sm:inline">Thêm Khẩu trừ</span>
        </Button>
      </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="hidden sm:flex gap-2 items-center">
              <Select value={filterStatus} onValueChange={(v: any) => setFilterStatus(v)}>
                ...
                <SelectContent>
                  <SelectItem value="all">Tất cả Khẩu trừ</SelectItem>
      ...
      <Card className="bg-black/50 border-purple-500/30">
        <CardHeader className="pb-3 px-4 sm:px-6 pt-4 sm:pt-6">
          <div className="flex items-center justify-between">
            ...
            >
              <Plus className="h-4 w-4 mr-2" /> Thêm Khẩu trừ
            </Button>
          </div>
        </CardHeader>
              ...
              <div className="sm:hidden space-y-2">
                {paginatedData.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-white/50 text-sm">
                    {hasActiveFilters
                      ? "Không tìm thấy Khẩu trừ nào phù hợp"
                      : "Chưa có Khẩu trừ nào"}
                  </div>
                )}
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-purple-500/30">
                      <TableHead className="text-white">Tên Khẩu trừ</TableHead>
                    ...
                    {paginatedData.length > 0 ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-8 text-white/60">
                          {hasActiveFilters
                            ? "Không tìm thấy Khẩu trừ nào phù hợp với bộ lọc"
                            : "Không tìm thấy Khẩu trừ nào"}
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
        ...
        <DialogContent className="bg-black/90 border-purple-500/30 text-white w-[calc(100vw-2rem)] max-w-lg mx-auto rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl font-bold text-white">
              {editingAllowance ? "Chỉnh Sửa Khẩu trừ" : "Thêm Khẩu trừ Mới"}
            </DialogTitle>
          </DialogHeader>
            ...
            >
              {creating ? (
                ...
                <>
                  ...
                  {editingAllowance ? "Cập Nhật" : "Tạo Khẩu trừ"}
                </>
              )}
            </Button>
          </div>
        ...
        <DialogContent className="bg-black/90 border-red-500/30 text-white w-[calc(100vw-2rem)] max-w-md mx-auto rounded-2xl">
          ...
          <div className="space-y-4">
            <p className="text-white/80 text-sm">
              Bạn có chắc chắn muốn xóa Khẩu trừ{" "}
