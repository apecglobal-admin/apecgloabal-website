...
import { ArrowLeft, Save, Eye, X, Plus, FileText, Settings } from "lucide-react";
...
export default function CreateNewsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/internal/admin/news');
    } catch (error) {
                ...
                {formData.tags.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {formData.tags.map((tag, index) => (
                      <Badge key={index} variant="secondary" className="bg-purple-500/20 text-purple-300 border-purple-500/30 flex items-center gap-1">
                        {tag}
                        <X 
